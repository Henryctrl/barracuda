...
export function MapComponent({ activeView, isSearchMode, setIsSearchMode }: MapComponentProps) {
  ...
  useEffect(() => {
    ...
    newMap.on('load', () => {
      ...
      newMap.on('click', 'parcelles-fill', handleClick);
    });
    
    return () => { if (map.current) { map.current.remove(); map.current = null; } };
  }, [isSearchMode, addDataLayers, findDPE, findDVF, throttle]);
        ...
        return (
            <div>
                ...
                <div className="space-y-2">
                    {dpeResults.length > 0 && renderDpeItem(topResult, 0)}
          ...
          <div key={sale.idmutinvar} onClick={() => setHighlightedSaleParcels(sale.l_idpar)} className={`w-full text-left p-2 rounded-md transition-all cursor-pointer hover:bg-accent-yellow/20 focus:outline-none focus:ring-2 focus:ring-accent-yellow ${isHighlighted ? 'bg-accent-yellow/20 ring-2 ring-accent-yellow' : ''}`} role="button" tabIndex={0} > 
            <div className={`pt-2 ${index > 0 ? 'border-t border-dashed border-accent-cyan/30' : ''}`}> 
              ...
              <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm"> 
                ...
                {sale.l_idpar.length <= 1 && (<><span className="font-semibold text-text-primary/80">LAND AREA:</span><span className="font-bold text-white text-right">{sale.sterr ? `${parseFloat(sale.sterr)} m²` : 'N/A'}</span></> )}
