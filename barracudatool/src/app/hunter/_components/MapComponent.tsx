...
interface DPERecord {
  'numero_dpe': string; 'adresse_brut': string; 'nom_commune_ban': string; 'code_postal_ban': string;
  'etiquette_dpe': string; 'etiquette_ges': string; '_geopoint': string; '_distance'?: number;
}
interface DVFRecord {
...
export function MapComponent({ activeView }: MapComponentProps) {
  ...
  const findDPE = useCallback(async (postalCode: string, lat: number, lon: number) => {
    ...
    try {
      ...
      setDpeResults(data);
      setDpeSearchInfo(`ANALYSIS COMPLETE. ${data.length} VALID ASSETS. CLOSEST TARGET: ${Math.round(data[0]._distance!)}m`);
    } catch (err) { const msg = err instanceof Error ? err.message : 'Unknown DPE Error'; setDpeError(msg);
    } finally { setIsDpeLoading(false); }
  }, []);

  // --- DVF Search Logic (Simplified) ---
      ...
      newMap.on('click', 'parcelles-fill', async (e) => {
         if (e.features && e.features.length > 0) {
           ...
           if (parcelId) {
             ...
             try {
                // Cadastre API call remains the same
                ...
                if (addressFeatures.length > 0) {
                  const bestResult = addressFeatures.find(f => f.properties.housenumber) || addressFeatures[0];
                  setBanAddress(bestResult);
                  setOtherAddresses(addressFeatures.filter(f => f.properties.label !== bestResult.properties.label).slice(0, 5));
  ...
  const getDpeColor = (rating: string) => { switch (rating) { case 'A': return '#00ff00'; case 'B': return '#adff2f'; case 'C': return '#ffff00'; case 'D': return '#ffd700'; case 'E': return '#ffa500'; case 'F': return '#ff4500'; case 'G': return '#ff0000'; default: return '#808080'; } };
  ...
  const renderPanelContent = () => {
    ...
    switch (activeView) {
      ...
      case 'dpe': return ( <div> {isDpeLoading && <div className="flex items-center justify-center gap-2 text-accent-cyan"><Loader2 className="animate-spin" size={16} /><span>SCANNING DPE GRID...</span></div>} {dpeError && <div className="p-3 text-center font-bold bg-red-900/50 border border-red-500 text-red-400 rounded-md">{dpeError}</div>} {dpeSearchInfo && !isDpeLoading && <div className="p-3 text-center font-bold bg-cyan-900/50 border border-accent-cyan text-accent-cyan rounded-md">{dpeSearchInfo}</div>} {dpeResults.length > 0 && ( <div className="space-y-3"> {dpeResults.slice(0, 5).map((dpe, index) => ( <div key={dpe.numero_dpe} className={`pt-3 ${index > 0 ? 'border-t border-dashed border-accent-cyan/30' : ''}`}> <div className={`text-sm font-bold mb-2 ${index === 0 ? 'text-accent-magenta' : 'text-accent-cyan'}`}> TARGET #{index + 1}: {dpe.adresse_brut || 'Address not available'} </div> <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm"> <span className="font-semibold text-text-primary/80">PROXIMITY:</span><span className="font-bold text-white text-right">{Math.round(dpe._distance ?? 0)} meters</span> <span className="font-semibold text-text-primary/80">ENERGY:</span><span className="font-bold text-right" style={{ color: getDpeColor(dpe.etiquette_dpe) }}>{dpe.etiquette_dpe}</span> <span className="font-semibold text-text-primary/80">GHG:</span><span className="font-bold text-right" style={{ color: getDpeColor(dpe.etiquette_ges) }}>{dpe.etiquette_ges}</span> <span className="font-semibold text-text-primary/80">COMMUNE:</span><span className="font-bold text-white text-right">{dpe.nom_commune_ban}</span> <span className="font-semibold text-text-primary/80">ID:</span><span className="font-mono text-white text-right text-xs">{dpe.numero_dpe}</span> </div> </div> ))} </div> )} </div> );
      case 'sales': return ( <div> {isDvfLoading && <div className="flex items-center justify-center gap-2 text-accent-cyan"><Loader2 className="animate-spin" size={16} /><span>SCANNING DVF GRID...</span></div>} {dvfError && <div className="p-3 text-center font-bold bg-red-900/50 border border-red-500 text-red-400 rounded-md">{dvfError}</div>} {dvfSearchInfo && !isDvfLoading && <div className="p-3 text-center font-bold bg-cyan-900/50 border border-accent-cyan text-accent-cyan rounded-md">{dvfSearchInfo}</div>} {dvfResults.length > 0 && ( <div className="space-y-3"> {dvfResults.slice(0, 10).map((sale, index) => ( <div key={sale.idmutinvar} className={`pt-3 ${index > 0 ? 'border-t border-dashed border-accent-cyan/30' : ''}`}> <div className={`text-sm font-bold mb-2 ${index === 0 ? 'text-accent-magenta' : 'text-accent-cyan'}`}> SALE: {new Date(sale.datemut).toLocaleDateString()} </div> <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm"> <span className="font-semibold text-text-primary/80">PRICE:</span><span className="font-bold text-white text-right">{parseFloat(sale.valeurfonc).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' }) || 'N/A'}</span> <span className="font-semibold text-text-primary/80">TYPE:</span><span className="font-bold text-white text-right">{sale.libtypbien || 'N/A'}</span> <span className="font-semibold text-text-primary/80">BUILT AREA:</span><span className="font-bold text-white text-right">{sale.sbati ? `${parseFloat(sale.sbati)} m²` : 'N/A'}</span> <span className="font-semibold text-text-primary/80">LAND AREA:</span><span className="font-bold text-white text-right">{sale.sterr ? `${parseFloat(sale.sterr)} m²` : 'N/A'}</span> </div> </div> ))} </div> )} </div> );
      default: return null;
    }
  };
