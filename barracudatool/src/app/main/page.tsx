...
import MapComponent from '../../components/MapComponent' // Adjusted path for main page
import Button from '../../components/ui/Button'
import Card from '../../components/ui/Card'
import { useScrollLock } from '../../hooks/useScrollLock' // Adjusted path for main page
...
export default function Page() {
  // Activate scroll lock for the entire page
  useScrollLock(true);
  ...
  const formatEnergyClass = (energyClass: string) => {
    ...
    return (
      <span className={`px-2 py-1 rounded font-bold text-xs ${colors[energyClass] || 'bg-gray-500 text-white'}`}>
        {energyClass || 'N/A'}
      </span>
    );
  };
        ...
        {selectedProperty && (
          <Card neonColor="yellow" className="backdrop-blur-md flex-shrink-0">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button neonColor={activeSection === 'overview' ? "yellow" : "gray"} size="sm" variant={activeSection === 'overview' ? 'primary' : 'secondary'} onClick={() => setActiveSection('overview')} className="text-xs">
                📋 Overview
              </Button>
              <Button neonColor={activeSection === 'cadastral' ? "green" : "gray"} size="sm" variant={activeSection === 'cadastral' ? 'primary' : 'secondary'} onClick={() => setActiveSection('cadastral')} className="text-xs">
                🏠 Cadastral
              </Button>
              <Button neonColor={activeSection === 'dpe' ? "purple" : "gray"} size="sm" variant={activeSection === 'dpe' ? 'primary' : 'secondary'} onClick={() => setActiveSection('dpe')} className="text-xs">
                ⚡ Energy
              </Button>
              <Button neonColor={activeSection === 'sales' ? "orange" : "gray"} size="sm" variant={activeSection === 'sales' ? 'primary' : 'secondary'} onClick={() => setActiveSection('sales')} className="text-xs">
                💰 Sales
              </Button>
            </div>
          </Card>
