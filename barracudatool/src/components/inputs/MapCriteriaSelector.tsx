...
import { MapPin, Navigation, Trash2, Plus, Pencil, Circle } from 'lucide-react';
...
export default function MapCriteriaSelector({
  ...
  // Update visual layers when data changes
  useEffect(() => {
  ...
  // Polygon drawing for custom sectors
  ...
  const removePlace = (code: string) => {
    ...
    // Also remove associated radius searches
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        <div>
          ...
          {selectedPlaces.length === 0 ? (
            <div className="text-xs text-gray-400 italic">
              Search and click locations to add them
            </div>
          ) : (
      ...
      {mode === 'radius' && (
        <div>
          <div className="text-xs text-[#ff00ff] font-bold mb-2 uppercase">
            Radius Searches
          </div>
          {selectedPlaces.length === 0 ? (
            <div className="text-xs text-gray-400 italic">
